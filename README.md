...
FontFlow 的核心是一套分層文字分析引擎（位於 [`app/lib/fontHelper.ts`](app/lib/fontHelper.ts)），透過逐層檢測字符支援情況，生成多維度評分。

#### **第一層：基本關鍵字檢測 (35 字)**

這 35 個字是日文字型**最常缺少**的繁體字：
...
1. 逐字調用 OpenType.js 的 `cmap`（字符映射表）檢查
2. 統計支援字數與缺失字數
3. **計算覆蓋率** = 支援字數 / 35
...
- **權重 40%**（最高），因為這些字是最常用的
- **懲罰機制**：若缺字超過 20%（≥7 字），最終分數被強制限制為 ≤60%，即使其他層評分高也無法彌補
- **示例**：日文字型經常只缺「們、對、著」三個字，卻被誤認為支援度不錯——本機制防止此誤判

#### **第二層：核心繁體字檢測 (6,373 字)**

基於 JetBrains Font (JF7000) 的完整繁體字集，覆蓋日常排版需求。

**檢測邏輯：**

1. 加載預定義的 6,373 個繁體字列表
2. 對每個字符進行字型支援檢查
3. **計算覆蓋率** = 支援字數 / 6,373

**覆蓋率分級：**

- **95% 以上**：綠色 (`bg-green-500`) — 完美支援
- **75-94%**：藍色 (`bg-blue-500`) — 良好支援，個別缺字
- **50-74%**：琥珀色 (`bg-amber-600`) — 基本可用，多處缺字
- **50% 以下**：灰色 (`bg-stone-500`) — 不建議使用

**權重**：35%（僅次於關鍵字）

#### **第三層：進階字集檢測 (1,692 字)**

擴展支援的特定領域字符，平均分配 15% 權重：

| 子類別     | 字數      | 說明                     | 權重    |
| ---------- | --------- | ------------------------ | ------- |
| 粵語特有字 | 137       | 廣東話用字（唔、嘞、咯） | 3.75%   |
| 台灣字     | 930       | 台灣特有地名、方言       | 3.75%   |
| 人名常用字 | 625       | 人名排版（榮、哲、曾）   | 3.75%   |
| **小計**   | **1,692** | —                        | **15%** |

**檢測邏輯：**

1. 分類檢測，統計各子類別支援率
2. 若某子類別缺字過多，單獨標註（如「粵語支援度 45%」）
...
- 檢查簡體專用字（如「么、呢、了」等只在簡體出現的字）
- 若簡體字覆蓋率 > 70%，標記為「簡體優化字型」
- 防止誤用簡體字型做繁體排版

##### **日文 (Hiragana + Kanji + Katakana)**

- 檢查平假名（`ぁ-ん`，46 字）
- 檢查片假名（`ァ-ン`，46 字）
- 檢查日文漢字（常用 2,136 字）
- 若平假名 + 片假名 > 80% 且日文漢字 > 50%，判定為「日文字型」

##### **英文 (基本拉丁字母)**

- 檢查 A-Z、a-z、0-9
- 檢查常用符號（@、#、$、% 等）
- 覆蓋率計算：支援字數 / 95（基本英文字符）
...
1. **逐字對比**：將預覽文字與字型支援情況逐字檢查
2. **標記缺字**：無法顯示的字用**方形框**視覺標示
3. **統計缺字**：「預覽文字中共有 X 個字無法顯示」
4. **分類列表**：按字集層級列出所有缺字（可選展開查看詳細清單）

#### **性能優化**

為避免每次分析都重新計算，實現了以下優化：

| 優化方案       | 說明                                               |
| -------------- | -------------------------------------------------- |
| **字符緩存**   | 首次檢查的字符結果被緩存，重複字符直接查表         |
| **批量檢測**   | 使用 `requestIdleCallback` 分時段檢測，避免阻塞 UI |
| **漸進式加載** | 基本評分先返回，詳細清單後續異步加載               |
| **WebWorker**  | 大字集检测可委派給 Web Worker（保留接口）          |
...
本專案實現了完整的 SEO 優化，每個頁面都包含相應的元數據（Metadata）用於搜尋引擎和社交媒體分享。
...
- **標題**：「FontFlow - 繁體中文字型分析工具」
- **描述**：「專業的繁體中文字型相容性檢測工具。快速判斷日文、簡體字型對繁體中文的支援程度，掌握確切的缺字清單。」
- **關鍵字**：字型分析、字體檢測、繁體中文、字型相容性、缺字檢查、排版工具
- **Open Graph**：支援 Facebook、Twitter 等社交媒體卡片展示
...
**路徑**：`app/(pages)/analysis/page.tsx`

**元數據**：

- **標題**：「字型分析 - FontFlow」
- **描述**：「深入分析字型的繁體中文支援程度。上傳 TTF/OTF 字型，獲得詳細的字符覆蓋率報告和缺字清單。」
- **關鍵字**：字型分析、繁體中文支援、字符覆蓋率、缺字檢查、字型評估
...
**路徑**：`app/(pages)/comparison/page.tsx`

**元數據**：

- **標題**：「字型比較 - FontFlow」
- **描述**：「並排對比多個字型的繁體中文支援程度。同時上傳最多 3 個字型，快速比較字符覆蓋率和視覺呈現。」
- **關鍵字**：字型比較、字體對比、繁體中文、多字型對比、字符覆蓋率比較
...
// 頁面級 Metadata (app/(pages)/analysis/page.tsx)
export const metadata: Metadata = {
  title: '字型分析 - FontFlow',
  description: '...',
  keywords: [...]
...
- ✅ Open Graph 標籤支援社交媒體分享
- ✅ Structured Data（結構化資料）可選集成
- ✅ `robots.txt` 配置（可選）
- ✅ Sitemap 生成（可選）
...
- 設置 `export const dynamic = 'force-dynamic'` 防止靜態緩存
- 確保 SEO 爬蟲能獲取最新頁面內容
- 支援根據用戶輸入動態調整 Meta 描述（進階功能）

### **無障礙訪問 (A11y)**

所有頁面遵循 WCAG 2.1 指南：

- ✅ 語義化 HTML（`<section>`、`<article>`、`<header>`）
- ✅ ARIA 標籤用於複雜交互
- ✅ 鍵盤導航支援
- ✅ 高對比度配色（達到 AA 級別）
- ✅ 屏幕閱讀器友善
